...
//            Wallet stewardWallet = WalletUtils.openStewardWallet();
            ...
            pbKeyList.append(verkey);
            for( int i=1; i < 20; i++) {
                pbKeyList.append("|" + CryptoUtils.generateAndStoreKey(csWallet, "0000000000000000000000000CS-" + String.format("%04d", i)));
            }
            ...
            byte[] proofSignatureRaw  = Ring.sign(payload.getBytes(), "0000000000000000000000000CS-0000".getBytes(), pbKeyList.toString() );
            proofSignature = Base64.getEncoder().encodeToString(proofSignatureRaw);

            mCommonUtils.stopTimer();
            mCommonUtils.writeLine(proofSignatureRaw.length + " charlength: " + proofSignature.length() );
            mCommonUtils.writeLine("Signature Generation Ends");

            // Ring Signature verification

            if(! Ring.verify(payload.getBytes(), Base64.getDecoder().decode(proofSignature), pbKeyList.toString())) {
                System.exit(0);
            }
